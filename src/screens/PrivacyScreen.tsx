    ...
    <div className="fade-in" style={{ backgroundColor: '#1A1A2E', minHeight: '100vh', paddingBottom: '36px', color: '#FFFFFF' }}>
      ...
      <Modal isOpen={activeModal === 'terms'} onClose={() => setActiveModal(null)} title={translateText('Privacy Statement', language)}>
        <div style={{ maxHeight: '300px', overflowY: 'auto', fontSize: '13px', color: '#B3B3C2', lineHeight: '1.6', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <p style={{ margin: 0 }}><strong style={{ color: '#FFFFFF' }}>{language === 'ar' ? '١. جمع المعلومات:' : '1. Information Collection:'}</strong> {language === 'ar' ? 'يقوم تطبيق كيو تي باي بجمع بيانات الجهاز ورقم الهاتف للمصادقة الإلزامية الخاضعة لضوابط البنك المركزي السعودي وساريع.' : 'alph pay collects device information, SIM serial data, and mobile numbers for mandatory multi-factor authentication mandated by SAMA and the Sarie payment system.'}</p>
          <p style={{ margin: 0 }}><strong style={{ color: '#FFFFFF' }}>{language === 'ar' ? '٢. تشفير البيانات:' : '2. Data Encryption:'}</strong> {language === 'ar' ? 'جميع بيانات المعاملات والمدفوعات مؤمنة بتشفير عتادي متقدم TLS 1.3 و 256-bit AES.' : 'All transaction payload communication is secured via TLS 1.3 and 256-bit AES end-to-end hardware encryption.'}</p>
          <p style={{ margin: 0 }}><strong style={{ color: '#FFFFFF' }}>{language === 'ar' ? '٣. سياسة الجهات الخارجية:' : '3. Third Party Policy:'}</strong> {language === 'ar' ? 'لا نبيع بياناتك الشخصية مطلقاً. تتم مشاركة البيانات المالية فقط مع بنكك السعودي المعتمد لتنفيذ المعاملات وفق لوائح ساما.' : 'We never sell your personal data. Financial data is shared only with your authorized Saudi bank network for payment processing under SAMA regulations.'}</p>
        </div>
      </Modal>
    </div>
  );
