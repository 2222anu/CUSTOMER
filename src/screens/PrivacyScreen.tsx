  ...
  return (
    <div className="fade-in" style={{ backgroundColor: '#0B0B14', minHeight: '100vh', paddingBottom: '36px', color: '#FFFFFF' }}>
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ fontSize: '11px', fontWeight: 800, color: '#6E6E85', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px', paddingInlineStart: '4px' }}>
      ...
      <Modal isOpen={activeModal === 'preferences'} onClose={() => setActiveModal(null)} title={translateText('Data Sharing Preferences', language)}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px', backgroundColor: '#1E1E32', borderRadius: '14px', border: '1px solid #2C2C44' }}>
            <div>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#FFFFFF' }}>{translateText('Personalized Offers', language)}</div>
              <div style={{ fontSize: '11px', color: '#A2A2BA', marginTop: '2px' }}>{translateText('Allow curated cashback & reward recommendations', language)}</div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px', backgroundColor: '#1E1E32', borderRadius: '14px', border: '1px solid #2C2C44' }}>
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#A2A2BA', marginTop: '2px' }}>{translateText('Share anonymized spending statistics', language)}</div>
            </div>
      ...
      <Modal isOpen={activeModal === 'export'} onClose={() => setActiveModal(null)} title={translateText('Export Account Data', language)}>
        {exportSuccess ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ fontSize: '12px', color: '#A2A2BA', marginTop: '4px' }}>{translateText('Your encrypted CSV statement will be sent to your registered email.', language)}</p>
          </div>
        ) : (
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            <p style={{ fontSize: '13.5px', color: '#A2A2BA', marginBottom: '20px', lineHeight: '1.5' }}>
      ...
      <Modal isOpen={activeModal === 'terms'} onClose={() => setActiveModal(null)} title={translateText('Privacy Statement', language)}>
        <div style={{ maxHeight: '300px', overflowY: 'auto', fontSize: '13px', color: '#A2A2BA', lineHeight: '1.6', display: 'flex', flexDirection: 'column', gap: '10px' }}>
